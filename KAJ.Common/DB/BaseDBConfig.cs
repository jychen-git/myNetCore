...
namespace KAJ.Common.DB
{
    ...
    public class BaseDBConfig
    {
        ...
        public static (List<MutiDBOperate>, List<MutiDBOperate>) MutiInitConn()
        {
            ...
            // 单库，且不开启读写分离，只保留一个
            if (!Appsettings.app(new string[] { "CQRSEnabled" }).ToObjBool() && !Appsettings.app(new string[] { "MutiDBEnabled" }).ToObjBool())
            {
                ...
                {
                    var dbFirst = listdatabase.FirstOrDefault(d => d.ConnId == Appsettings.app(new string[] { "MainDB" }).ToObjString());
                    if (dbFirst == null)
                    {
            ...
            if (Appsettings.app(new string[] { "CQRSEnabled" }).ToObjBool() && !Appsettings.app(new string[] { "MutiDBEnabled" }).ToObjBool())
            {
                if (listdatabase.Count > 1)
                {
                    listdatabaseSlaveDB = listdatabase.Where(d => d.ConnId != Appsettings.app(new string[] { "MainDB" }).ToObjString()).ToList();
                }
            }
